...
class SEN12MSCRABase(Dataset):
    def __init__(self,
        ...
        self._length = len(self._dataset)
        self.flip_p = flip_p
        self.transform = transforms.Compose([transforms.Lambda(lambda img: torch.Tensor(img)),
    ...
    def __getitem__(self, i):
        ...
        S1 = torch.permute(self.transform(S1), (1, 2, 0))
        example['S1']=S1
        example['S1_path']=S1_path
...
class SEN12MSCRBBase(Dataset):
    def __init__(self,
        ...
        self.data_root = data_root
        self._dataset = SEN12MSCR_B(data_root, split=split, season=season, s1_rescale_method=s1_rescale_method,  s2_rescale_method=s2_rescale_method, s1_rgb_composite=s1_rgb_composite, per_image=per_image, Lambda=lambda_B)
        self._length = len(self._dataset)
        self.flip_p = flip_p
        self.transform = transforms.Compose([transforms.Lambda(lambda img: torch.Tensor(img)),
                                             transforms.RandomHorizontalFlip(p=flip_p),
    ...
    def __getitem__(self, i):
        ...
        S2 = torch.permute(self.transform(S2), (1, 2, 0))
        example['S2']=S2
...
class SEN12MSCRATrain(SEN12MSCRABase):
    def __init__(self, **kwargs):
        super().__init__(data_root="", split='train', season="", s1_rescale_method='linear', s2_rescale_method='linear', s1_rgb_composite='add')
        

class SEN12MSCRAValidation(SEN12MSCRABase):
    def __init__(self, flip_p=0., **kwargs):
        super().__init__(data_root="", split='test_use_all', season="", s1_rescale_method='linear', s2_rescale_method='linear', s1_rgb_composite='add', flip_p=flip_p)
        
    
class SEN12MSCRBTrain(SEN12MSCRBBase):
    def __init__(self, **kwargs):
        super().__init__(data_root="", split='train', season="", s1_rescale_method='linear', s2_rescale_method='linear', s1_rgb_composite='add')
        

class SEN12MSCRBValidation(SEN12MSCRBBase):
    def __init__(self, flip_p=0., **kwargs):
        super().__init__(data_root="", split='test_use_all', season="", s1_rescale_method='linear', s2_rescale_method='linear', s1_rgb_composite='add', flip_p=flip_p)
# 
# 
