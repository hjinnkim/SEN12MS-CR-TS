...
def get_transform(opt, params=None, grayscale=False, method=InterpolationMode.BICUBIC):
    transform_list = []
    if grayscale:
    ...
    if not opt.no_flip:
        if params is None:
            ...
            transform_list.append(transforms.Lambda(lambda img: __hflip(img, params['flip'][0])))
            transform_list.append(transforms.Lambda(lambda img: __vflip(img, params['flip'][1])))
...
def get_params(opt, size):
    ...
    return {'crop_pos': (x, y), 'flip': (flip1, flip2)}
